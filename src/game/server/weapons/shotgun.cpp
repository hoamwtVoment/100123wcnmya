...
void CShotgun::Fire(vec2 Direction)
{
	...
	bool ExtraEnabled = true;
		...
		for(int i = -ShotSpread; i <= ShotSpread; ++i)
		{
			...
			SBulletMode Mode = Character()->Controller()->GetBulletMode(WEAPON_SHOTGUN, vec2(cosf(a), sinf(a)) * Speed, Server()->Tick(), Character()->CurrentTuning()->m_ShotgunSpeed);
			int PelletLifetime = Mode.m_LifeSpan > 0 ? Mode.m_LifeSpan : Lifetime;
