...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if(Server()->ClientIngame(ClientID))
	{
		...
		{
			dbg_msg("tkt-debug", "CL_KILL from client %d", ClientID);
			if(!pPlayerWorld || pPlayerWorld->m_Paused)
				return;
...
void CGameContext::OnConsoleInit()
{
	...
	Console()->Register("allweapons", "?i[id]", CFGFLAG_SERVER, ConAllWeapons, this, "Give all weapons to a player, no argument gives them to the caller");
...
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int ClientID = pResult->NumArguments() ? pResult->GetInteger(0) : pResult->m_ClientID;
	...
	pChr->GiveWeapon(WEAPON_GUN, WEAPON_ID_PISTOL, g_pData->m_Weapons.m_aId[WEAPON_GUN].m_Maxammo);
	pChr->GiveWeapon(WEAPON_SHOTGUN, WEAPON_ID_SHOTGUN, g_pData->m_Weapons.m_aId[WEAPON_SHOTGUN].m_Maxammo);
	pChr->GiveWeapon(WEAPON_GRENADE, WEAPON_ID_GRENADE, g_pData->m_Weapons.m_aId[WEAPON_GRENADE].m_Maxammo);
	pChr->GiveWeapon(WEAPON_LASER, WEAPON_ID_LASER, g_pData->m_Weapons.m_aId[WEAPON_LASER].m_Maxammo);
}
