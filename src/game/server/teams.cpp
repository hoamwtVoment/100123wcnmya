...
void CGameTeams::OnPlayerConnect(CPlayer *pPlayer)
{
	...
	SGameInstance Instance = GetPlayerGameInstance(ClientID);
	if(Instance.m_IsCreated && Instance.m_pController)
		Instance.m_pController->OnInternalPlayerJoin(pPlayer, INSTANCE_CONNECTION_SERVER);
...
void CGameTeams::OnPlayerDisconnect(CPlayer *pPlayer, const char *pReason)
{
	...
	SGameInstance Instance = GetPlayerGameInstance(ClientID);
	if(Instance.m_IsCreated && Instance.m_pController)
		Instance.m_pController->OnInternalPlayerLeave(pPlayer, INSTANCE_CONNECTION_SERVER);
