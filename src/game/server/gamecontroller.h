...
// bullet behavior for projectile weapons, queried via IGameController::GetBulletMode
struct SBulletMode
{
	SBulletMode(vec2 Direction) :
		m_pCallback(nullptr),
		m_pData(nullptr),
		m_pDestroyData(nullptr),
		m_Direction(Direction),
		m_Bouncy(false),
		m_LifeSpan(0) {}

	// null -> the weapon uses its own default impact callback
	bool (*m_pCallback)(class CProjectile *, vec2, class CCharacter *, bool);
	// opaque per-bullet state, only valid when m_pCallback is set
	void *m_pData;
	// cleanup for m_pData, called when the projectile is destroyed
	void (*m_pDestroyData)(void *);
	// effective flight direction (already scaled by the custom speed)
	vec2 m_Direction;
	// snap the projectile as a bouncy projectile
	bool m_Bouncy;
	// per-bullet lifetime in ticks, 0 = use the weapon's default
	int m_LifeSpan;
};
...
/*
	Function: GetBulletMode
		Called when a projectile weapon fires, asks the controller which
		bullet behavior this room wants.

	Arguments:
		WeaponType - The weapon slot that fired (WEAPON_GUN/SHOTGUN/GRENADE)
		Direction - The aim direction, scale it to change the bullet speed
		StartTick - The current server tick
		TuneSpeed - The weapon's tuning speed, used to convert a custom
				speed into a direction scale

	Result:
		An SBulletMode. A null m_pCallback means the weapon keeps its
		default (vanilla) behavior and no custom data is allocated.
*/
virtual SBulletMode GetBulletMode(int WeaponType, vec2 Direction, int StartTick, float TuneSpeed) { return SBulletMode(Direction); };
